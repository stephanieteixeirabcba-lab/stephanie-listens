...
// ── REMOTE BCBA GUIDE FORM ───────────────────────────
const remoteForm = document.getElementById('remote-form')
if (remoteForm) {
  remoteForm.addEventListener('submit', async (e) => {
    e.preventDefault()
    await submitEmail({
      email: document.getElementById('remote-email').value.trim(),
      firstName: document.getElementById('remote-first-name')?.value.trim() || '',
      submitBtn: document.getElementById('remote-submit-btn'),
      messageEl: document.getElementById('remote-form-message'),
      formEl: remoteForm,
      source: 'remote-bcba-page',
      tag: 'remote-bcba-questions'
    })
  })
}
